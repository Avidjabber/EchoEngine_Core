...
import type { WeatherStateSavedRow, WeatherStateOverwrittenRow, RowError } from '../../../../services/model/weatherStatePackService';
...
export function buildUploadMessages(
    ...
    if (isEmpty) {
        ...
        if (newCount > 0)       annoLines.push(`**${newCount}** weather state${newCount === 1 ? '' : 's'} added`);
        if (overwriteCount > 0) annoLines.push(`**${overwriteCount}** weather state${overwriteCount === 1 ? '' : 's'} updated`);
    ...
    if (newSaved.length > 0) {
        const lines = [
            '## Added',
            ...newSaved.map(r => `-# ${r.codeName} | ${r.name}${r.isSevere ? ' | severe' : ''}`),
        ];
    ...
    if (overwriteCount > 0) {
        const lines = [
            ...
            ...overwrites.map(r => {
                ...
                ].filter(Boolean).join(', ') || 'env conditions updated';
