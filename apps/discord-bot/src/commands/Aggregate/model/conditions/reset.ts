...
import { resetConditionPack } from '../../../../services/model/conditionPackService';
...
export async function execute(interaction: ChatInputCommandInteraction): Promise<void> {
                ...
                components:   [
                    {
                        ...
                        content: [
                            '## Reset Conditions',
                            'This will permanently delete **all guild-defined** condition definitions for this guild.',
                            '-# Engine-owned conditions (global defaults) are preserved and cannot be removed.',
                            '-# Conditions still referenced by active entities or other records cannot be removed.',
                    ...
                    },
                    {
                        type:       1,
                        components: [
                            { type: 2, style: 4, label: 'Confirm Reset', custom_id: 'conditions_reset_confirm' },
                            { type: 2, style: 2, label: 'Cancel',        custom_id: 'conditions_reset_cancel'  },
                        ],
                    },
                ],
            },
    ...
    if (confirmation.customId === 'conditions_reset_cancel') {
        await interaction.deleteReply();
        return;
    }
    ...
    const result  = await resetConditionPack(guildId);

    if (!result.success) {
        await replyError(interaction, 'Failed to reset conditions. Please try again.');
        return;
    }
    ...
    if (deleted.length === 0 && failed.length === 0) {
        annoLines.push('-# No guild-defined conditions were configured — nothing was cleared.');
    } else {
        if (deleted.length > 0) annoLines.push(`**${deleted.length}** condition${deleted.length === 1 ? '' : 's'} deleted`);
        if (failed.length > 0)  annoLines.push(`**${failed.length}** condition${failed.length === 1 ? '' : 's'} could not be removed`);
    }
