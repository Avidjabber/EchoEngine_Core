...
import type { ItemSavedRow, ItemOverwrittenRow, RowError } from '../../../../services/model/itemPackService';
...
export function buildUploadMessages(
    ...
    if (isEmpty) {
        ...
        if (newCount > 0)       annoLines.push(`**${newCount}** item${newCount === 1 ? '' : 's'} added`);
        if (overwriteCount > 0) annoLines.push(`**${overwriteCount}** item${overwriteCount === 1 ? '' : 's'} updated`);
    ...
    if (newSaved.length > 0) {
        const lines = [
            '## Added',
            ...newSaved.map(r => `-# ${r.codeName} | ${r.name}`),
        ];
    ...
    if (overwriteCount > 0) {
        const lines = [
            ...
            ...overwrites.map(r => {
                ...
                const detail = nameChanged ? `name: ${r.oldName} → ${r.newName}` : 'definition updated';
